...
export function TasksPage({ isDesktop, user, tasks, supervisors, technicians, staffs, onDone }: { isDesktop: boolean; user: User; tasks: Task[]; supervisors: User[]; technicians: User[]; staffs: User[]; onDone: () => Promise<void>; }) {
  ...
  const uploadDocumentationFromFile = async (file?: File | null) => {
    if (!file) return null;
    try {
      setUploadingDoc(true);
      const r = await api.uploadTaskDocumentation(file) as { documentation_image_url?: string };
  ...
  async function submitTask() {
    ...
    if (!form.title.trim() || !form.location.trim() || ((user.role === "supervisor" || user.role === "staff") && !form.supervisor_id)) { setFormError("Judul, lokasi, dan supervisor wajib diisi."); return; }
    try {
      setSaving(true);
      await api.createTask({
        ...form,
        supervisor_id: toIntOrUndefined(form.supervisor_id),
        staff_id: (user.role === "supervisor" || user.role === "staff" || user.role === "teknisi" || user.role === "technician") ? undefined : toIntOrUndefined(form.staff_id),
        technician_id: (user.role === "teknisi" || user.role === "technician") ? undefined : toIntOrUndefined(form.technician_id),
        documentation_image_url: form.documentation_image_url?.trim() || undefined, completion_percent: Number(form.completion_percent || 0),
      });
      await onDone();
      setFormSuccess("Tugas berhasil disimpan.");
      showToast("Tugas berhasil ditambahkan.");
      setForm({ title: "", description: "", customer: "", location: "", priority: "medium", supervisor_id: supervisors[0] ? String(supervisors[0].id) : "", staff_id: "", technician_id: "", documentation_image_url: "", due_date: "", completion_percent: "0" });
