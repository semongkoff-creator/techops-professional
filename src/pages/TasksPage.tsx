...
export function TasksPage({ isDesktop, user, tasks, supervisors, technicians, staffs, onDone }: { isDesktop: boolean; user: User; tasks: Task[]; supervisors: User[]; technicians: User[]; staffs: User[]; atasans?: User[]; onDone: () => Promise<void>; }) {
  ...
  const updateGalleryInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const [updateUploadedUrl, setUpdateUploadedUrl] = useState<string>("");
  const [nativeTarget, setNativeTarget] = useState<"create" | "edit" | "update" | null>(null);
  const isNativeUploadingUpdate = nativeTarget === "update";
  ...
  const validateMediaFile = (file?: File | null) => {
    ...
    const allowedVideoMime = new Set(["video/mp4", "video/webm", "video/quicktime", "video/ogg"]);
    const allowedImageExt = new Set(["jpg", "jpeg", "png", "webp"]);
    const allowedVideoExt = new Set(["mp4", "mov", "webm", "ogg"]);
    const isImage = allowedImageMime.has(type) || allowedImageExt.has(ext);
    const isVideo = allowedVideoMime.has(type) || allowedVideoExt.has(ext);
    if (!isImage && !isVideo) {
      return "Invalid file type. Allowed: jpg, jpeg, png, webp, mp4, webm, mov, ogg";
    }
    const maxBytes = isVideo ? 25 * 1024 * 1024 : 5 * 1024 * 1024;
    if (file.size > maxBytes) return isVideo ? "Ukuran video maksimal 25MB." : "Ukuran gambar maksimal 5MB.";
    return "";
  };
    ...
    const unregister = registerNativeBridgeListeners(
      ...
      (payload) => {
        ...
        if (nativeTarget === "update") {
          ...
          setUpdateUploadedUrl(uploadedUrl);
          if (selectedTask) {
