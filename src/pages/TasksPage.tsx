...
export function TasksPage({ isDesktop, user, tasks, supervisors, technicians, atasans, onDone }: { isDesktop: boolean; user: User; tasks: Task[]; supervisors: User[]; technicians: User[]; atasans: User[]; onDone: () => Promise<void>; }) {
  const [form, setForm] = useState({ title: "", description: "", location: "", priority: "medium", supervisor_id: "", technician_id: "", due_date: "", completion_percent: "0" });
  ...
  const canCreateTask = user.role === "atasan" || user.role === "supervisor" || user.role === "teknisi";
  ...
  const atasanName = (id?: number | null) => atasans.find((a) => a.id === id)?.name || "-";
  ...
  const action = (t: Task) => (
    <div className={`d-flex flex-wrap gap-2 ${isDesktop ? "desktop-action-wrap" : ""}`}>
      {canAssign && (() => {
        const alreadyAssigned = Boolean(t.technician_id) || ["assigned_to_technician", "in_progress", "completed", "closed"].includes(t.status);
        if (alreadyAssigned) {
          return null;
        }
        const techField = (
        ...
        if (isDesktop) {
          return <div className="desktop-supervisor-fields">
            <div className="desktop-tech-field">{techField}</div>
            <div className="desktop-assign-field">{assignField}</div>
          </div>;
        }
        return <>{techField}{assignField}</>;
      })()}
      {user.role === "teknisi" && <>
      ...
      <div className={`task-more-wrap ${isDesktop ? "desktop-more" : ""}`}>
        ...
        {activeTaskMenuId === t.id && (
          <div className={`task-inline-menu compact ${isDesktop ? "desktop-action-menu open-down" : ""}`}>
            ...
            {(user.role === "atasan" || user.role === "supervisor") && <button type="button" onClick={() => { setEditTask({ ...t }); setActiveTaskMenuId(null); }}><Pencil size={14} /> Edit</button>}
            {(user.role === "atasan" || user.role === "supervisor") && <button type="button" className="danger" onClick={() => { setConfirmDeleteTask(t); setActiveTaskMenuId(null); }}><Trash2 size={14} /> Hapus</button>}
          </div>
        )}
      </div>
    </div>
  ...
  async function submitTask() {
    ...
    if (!form.title.trim() || !form.location.trim() || !form.supervisor_id) { setFormError("Judul, lokasi, dan supervisor wajib diisi."); return; }
    try {
      setSaving(true);
      ...
      setForm({ title: "", description: "", location: "", priority: "medium", supervisor_id: supervisors[0] ? String(supervisors[0].id) : "", technician_id: "", due_date: "", completion_percent: "0" });
  ...
  async function submitEditTask() {
    if (!editTask) return;
    try {
      await api.updateTask(editTask.id, { title: editTask.title, description: editTask.description, location: editTask.location, priority: editTask.priority, supervisor_id: editTask.supervisor_id, due_date: editTask.due_date, completion_percent: editTask.completion_percent });
  ...
  return (
    <section className="d-grid gap-3">
      ...
      {isDesktop ? (
        <div className="card"><div className="card-body"><div className="d-flex justify-content-between align-items-center mb-2"><h5 className="mb-0">Job Activity Table</h5>{canCreateTask && <button className="btn btn-primary rounded-pill px-3" onClick={() => setShowCreateModal(true)}><Plus size={16} /> Buat Tugas</button>}</div><div className="worksheet-head"><div><strong>UNIT</strong></div><div className="worksheet-brand">SATRIA PIRANTI PERKASA</div><div className="text-end"><span className="badge text-bg-primary">PCS</span></div></div><div className="table-wrap desktop-job-table"><table className="table align-middle"><thead><tr className="worksheet-group"><th colSpan={2}>IDENTITAS</th><th colSpan={4}>AKTIVITAS</th><th colSpan={5}>PROGRESS</th><th colSpan={3}>PENUGASAN</th><th>AKSI</th></tr><tr><th>No</th><th>Tgl</th><th>Job</th><th>Detail Job</th><th>Customer</th><th>Lokasi</th><th>Plan</th><th>Aging</th><th>Status</th><th>Progress</th><th>Spareparts</th><th>Mekanik</th><th>Final</th><th>Keterangan</th><th>Aksi</th></tr></thead><tbody>{tasks.map((t, i) => <tr key={t.id}><td>{i + 1}</td><td>{formatDate(t.due_date || "")}</td><td>{t.priority?.toUpperCase() || "-"}</td><td>{t.title}</td><td>{technicianName(t.technician_id)}</td><td>{t.location || "-"}</td><td>{formatDate(t.due_date || "")}</td><td>{t.due_date ? Math.max(0, Math.ceil(((new Date(t.due_date).getTime() - Date.now()) / 86400000))) : "-"}</td><td><span className="badge text-bg-light border">{t.status}</span></td><td>{Math.max(0, Math.min(100, t.completion_percent || 0))}%</td><td>-</td><td>{technicianName(t.technician_id)}</td><td>{t.status === "completed" || t.status === "closed" ? "CLOSE" : "OPEN"}</td><td>{t.description || "-"}</td><td>{action(t)}</td></tr>)}</tbody></table></div></div></div>
      ) : (
